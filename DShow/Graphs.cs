// $Id: Graphs.cs 1985 2013-11-02 07:51:13Z onuchin $
// Author: Valeriy Onuchin   29.01.2013
...
namespace P.DShow
{
    ...
    public abstract class CaptureGraph
    {
        ...
        protected CheckPosFilter transform;
        ...
        public CheckPosFilter CheckPosFilter
        {
           get { return transform; }
        }

        public CheckPosFilter Transform
        {
           get { return transform; }
        }
        ...
        public virtual void AddCompressor(FilterInfo fiCompressor)
        {
            ...
            try {
               iGB.Connect((transform != null ? transform.OutputPin : source.OutputPin),
                           compressor.InputPin);
            } catch(COMException) {
                RemoveCompressor();
                throw;
        ...
        public virtual void AddCompressorNotDispose(FilterInfo fiCompressor, IPin source)
        {
           //RemoveCompressor();
           ...
           try {
              iGB.Connect(source, compressor.InputPin);
           } catch (COMException) {
              RemoveCompressor();
              throw;
           }
        ...
        {
           Filter start = transform != null ? (Filter)transform : source;
        ...
        public void RemoveFiltersDownstreamFrom(Filter start)
        {
            ...
            foreach(IBaseFilter iBF in toRemove) {
                ...
                if ((Transform != null) && (iBF == Transform.BaseFilter)) {
                   DisposeTransform();
                }
            }
        ...
        private bool FilterHasInputPin(IBaseFilter thisFilter, IPin inputPin) {
         ...
         public virtual void RenderStream(Guid categoryGuid, Guid typeGuid, object source, 
                                          Filter compressor, Filter renderer)
         {
            if (iGraphBuilder2 == null) {
                iGraphBuilder2  = CaptureGraphBuilder2Class.CreateInstance();
               iGraphBuilder2.SetFiltergraph(iGB);
            }

            byte[] dataPin = Pin.PIN_CATEGORY_PREVIEW.ToByteArray();
            GCHandle pin = GCHandle.Alloc(dataPin, GCHandleType.Pinned);

            byte[] dataType = MediaType.MajorType.MEDIATYPE_Video.ToByteArray();
            GCHandle type = GCHandle.Alloc(dataPin, GCHandleType.Pinned);

            try {
               IntPtr ptrType = Marshal.UnsafeAddrOfPinnedArrayElement(dataType, 0);
               IntPtr ptrPin = Marshal.UnsafeAddrOfPinnedArrayElement(dataPin, 0);

               iGraphBuilder2.RenderStream(ptrPin, ptrType, source, compressor.BaseFilter, renderer.BaseFilter);
            } finally {
               pin.Free();
               type.Free();
            }
        ...
        public virtual void RenderLocal()
        {
            ...
            IPin pin = compressor != null ? (IPin)compressor.OutputPin : (transform != null ? transform.OutputPin : source.OutputPin);
            iGB.Render(pin);
        }
        ...
        public void RenderNetwork(RtpSender rtpSender)
        {
            ...
            iGB.Connect(compressor != null ? compressor.OutputPin : (transform != null ? transform.OutputPin : source.OutputPin),
                        renderer.InputPin);
        }
    ...
    public class VideoCaptureGraph : CaptureGraph, IVideoCaptureGraph
    {
        ...
        public void AddCheckPosFilter()
        {
           transform = Filter.CheckPosFilter();
           iGB.AddFilter(transform.BaseFilter, transform.FriendlyName);
           transform.AddedToGraph(fgm);
           iGB.Connect(source.OutputPin, transform.InputPin);
        }
    ...
    public class DVCaptureGraph : CaptureGraph, IVideoCaptureGraph, IAudioCaptureGraph {
        ...
        public override void AddCompressor(FilterInfo fiCompressor) {
            ...
            if (fiCompressor.Category.Equals(Filter.CLSID_AudioCompressorCategory)) {
                ...
                try {
                    iGB.Connect(splitterAudioOut, audioCompressor.InputPin);
                }
                catch (COMException) {
                    RemoveAndDispose(audioCompressor);
                    audioCompressor = null;
                ...
                try {
                    iGB.Connect(splitterVideoOut, videoCompressor.InputPin);
                }
                catch (COMException) {
                    RemoveAndDispose(videoCompressor);
                    videoCompressor = null;
        ...
        public void GetVideoMediaType(out _AMMediaType mt, out object formatBlock) {
            ...
            Pin.GetMediaTypes(splitterVideoOut, out mts, out formats);
        ...
        public void GetAudioMediaType(out _AMMediaType mt, out object formatBlock) {
            ...
            Pin.GetMediaTypes(splitterAudioOut, out mts, out formats);
        ...
        public void GetAudioMediaTypes(out _AMMediaType[] mts, out object[] fbs) {
            ...
            Pin.GetMediaTypes(splitterAudioOut, out mts, out fbs);
        }
        ...
        private bool AddDVSplitter() {
            ...
            try {
                ...
                iGB.Connect((transform != null? transform.OutputPin : source.OutputPin), dvSplitterInput);
            }
            catch (COMException) {
                dvSplitter = null;
            ...
            //Find output pins
            try {
                splitterVideoOut = Filter.GetPin(dvSplitter, _PinDirection.PINDIR_OUTPUT, Guid.Empty, Guid.Empty, true, 0);
                splitterAudioOut = Filter.GetPin(dvSplitter, _PinDirection.PINDIR_OUTPUT, Guid.Empty, Guid.Empty, true, 1);
            }
            catch (COMException) {
                dvSplitter = null;
                splitterVideoOut = splitterAudioOut = null;
