...
namespace P.Net.Rtp
{
    ...
    public class SdesData : IRtcpData
    {
        ...
        /// <summary>
        /// Name and Version of tool sending streams, e.g. 'PROTOM v1.0'
        /// </summary>
        public string Tool
        {
        ...
        public void SetTool(bool val)
        {
            ...
            if(val)
            {
                ...
                tool  = String.Format("{0} v{1}, ", exe.Name, exe.Version);
                tool += String.Format("{0} v{1}", rtp.Name, rtp.Version);
            }
        ...
        public override string ToString()
        {
            ...
            if(privs != null)
            {
                foreach (DictionaryEntry de in privs)
                {
                    ...
                    str += String.Format(", {0}: {1}", prefix, data);
                }
            }
        ...
        private void WritePrivatePropertyToBuffer(byte[] prefix, byte[] data, BufferChunk buffer)
        {
            ...
            Debug.Assert(prefixLength + dataLength <= MAX_PRIV_PROPERTY_LENGTH);
