...
namespace P.Net.Rtp
{
                              ...
                              IDisposable
    {
       ...
       public static bool DiagnosticsEnabled;
        ...
        /// </summary>
       public static string DiagnosticsServer;
        ...
        private static readonly string DIAGNOSTICS_PORT_KEY = "P.DiagnosticPort";
        ...
        /// uninstall, be sure to delete HKLM\SOFTWARE\PROTOM\RtpInstalled before reinstalling Rtp in another location.  The clean way to
        /// uninstall without MSIs or Add/Remove Programs is to call "installutil /u P.Rtp.dll" before deleting the file or by having your
        /// app call Installer.Uninstall on the P.Rtp assembly programatically.  See Installation.cs for an example of how to programmatically
        ...
        /// </summary>
        private RtcpListener rtcpListener;
        ...
        /// </summary>
        private PacketTransform packetTransform;
        ...
        private void InitializeThreads()
        {
            ...
            {
                threadPerf = new Thread(UpdatePerformanceCountersThread);
        ...
        private void InitializePerformanceCounters()
        {
            ...
            if (BasePC.PerformanceCounterWrapper.HasUserPermissions())
            {
                lock(pcLock)
                {
                    if(participant != null)
                    {
                        pc = new RtpSessionPC(participant.CName + " - " + nextHopEP);
                    }
                }
            }
        }
        ...
        /// 
        ///  always stores the Participant ssrc/SdesData first in the SdesPacket.  So a
            ...
            lock(participants)
            {
                lock(streamsAndIPs)
                {
                    ...
                    if(ipsp.stream == null)
                    {
                        ...
                        ipsp.stream.PacketTransform = packetTransform;
                ...
                if(streamsAndIPs.Contains(ssrc))
                {
                    if(!ip.Equals(streamsAndIPs[ssrc].senderIP))
                    {
                        eventLog.WriteEntry(string.Format(CultureInfo.CurrentCulture, 
                            Strings.IPAddressTryingToRemoveString, ip, ssrc, 
                            streamsAndIPs[ssrc].senderIP), EventLogEntryType.Error, 
        ...
        private void ProcessPacket(RtcpPacket packet, IPAddress ipAddress)
        {
            switch(packet.PacketType)
            {
                ...
                {
                    eventLog.WriteEntry(string.Format(CultureInfo.CurrentCulture, 
                        Strings.ReceivedAnUnhandledRtcpType, packet), EventLogEntryType.Warning, 
        ...
        private void ProcessSDESPacket(SdesPacket packet, IPAddress ipAddress)
        {
            foreach(SdesReport report in packet.Reports())
            {
                ...
                else // Doesn't have  specific extensions
                {
        ...
        private void ProcessSRPacket(SrPacket packet, IPAddress ipAddress)
        {
            ...
            // TODO - Need SenderReportReceived(RtpStream, SenderReport)
        }
        ...
        private void HandleSSRCConflict(uint ssrc, IPAddress ipAddress)
        {
            ...
            if(rtpSenders.ContainsKey(ssrc))
            {
                ...
                eventLog.WriteEntry(string.Format(CultureInfo.CurrentCulture, 
                    Strings.SSRCConflictDetectedLocalSender, ipAddress), EventLogEntryType.Warning, 
            ...
            else
            {
                ...
                eventLog.WriteEntry(string.Format(CultureInfo.CurrentCulture, 
                    Strings.SSRCConflictDetectedRemoteMachines, ipAddress), EventLogEntryType.Warning, 
                    (int)RtpEL.ID.SSRCConflictDetected);
            }     
        }
        ...
        private void CheckForCNameConflict(string cname, IPAddress[] ipAddresses)
        {
            ...
            if (!ipAddresses[0].Equals(ipAddresses[1]))
            {
                ...
                if (participant.CName == cname)
                {
                    ...
                    ThreadPool.QueueUserWorkItem(Dispose);
                }
        ...
        CompoundPacketBuilder RtcpSender.IRtpSession.RtcpReportIntervalReached()
        {
            ...
            if ((VenueName != null) && (groupEP != null))
            {
                string appPayload = VenueName + "#" + groupEP.Address;
        ...
        private int networkTimeoutEvents;
        ...
        internal void RaiseNetworkTimeoutEvent(string source)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseNetworkTimeoutEvent, args);
        }
        ...
        internal void RaiseParticipantAddedEvent(RtpParticipant participant)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseRtpParticipantAddedEvent, args);
        }
        ...
        {
            object[] args = {this, new RtpEvents.RtpParticipantEventArgs(participant)};
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseRtpParticipantRemovedEvent, args);
        }
        ...
        {
            object[] args = {this, new RtpEvents.RtpParticipantEventArgs(participant)};
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseRtpParticipantTimeoutEvent, args);
        }
        ...
        internal void RaiseRtpStreamAddedEvent(RtpStream stream)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseRtpStreamAddedEvent, args);
        }
        ...
        {
            object[] args = {this, new RtpEvents.RtpStreamEventArgs(stream)};
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseRtpStreamRemovedEvent, args);
        }
        ...
        private int rtpStreamTimeoutEvents;
        ...
        internal void RaiseRtpStreamTimeoutEvent(RtpStream stream)
        {
            ...
            object[] args = {this, new RtpEvents.RtpStreamEventArgs(stream)};
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseRtpStreamTimeoutEvent, args);
        }
        ...
        private void RaiseReceiverReportEvent(uint rrSSRC, uint dataSourceSSRC, ReceiverReport rr)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseReceiverReportEvent, args);
        }
        ...
        private void RaiseAppPacketReceivedEvent(uint ssrc, string name, byte subtype, byte[] data)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseAppPacketReceivedEvent, args);
        }
        ...
        private void RaiseSenderReportReceivedEvent(uint ssrc,SenderReport senderReport)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseSenderReportEvent, args);
        }
        ...
        private void RaiseDuplicateCNameDetectedEvent(IPAddress[] ipAddresses)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseDuplicateCNameDetectedEvent, args);
        }
        ...
        private int invalidPacketEvents;
        ...
        private void RaiseInvalidPacketEvent(string msg)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseInvalidPacketEvent, args);
        }
