...
namespace P.Net.Rtp
{
    ...
    public class RtpStream
    {
        ...
        /// </summary>
        private bool nextFrameUnblocked;
        ...
        private BufferChunk lastFrame;
        private BufferChunk firstFrame;
        ...
        private int bytesReceivedLast;
        
        private int framesLost;
        private int framesReceived;
        private int framesReceivedLast;

        private int invalidPacketInFrameEvents; // 

        internal int packetsLost = 0;
        internal uint packetsReceived = 0;
        private uint packetsReceivedLast;
        private uint packetsReceivedLate;
        ...
        internal RtpStream(RtpListener rtpListener, uint ssrc, SdesData sdes, PayloadType pt)
        {
            ...
            returnBufferHandler = rtpListener.ReturnBufferCallback;
            this.ssrc = ssrc;
            this.pt = pt;
        ...
        internal virtual void ProcessPacket(RtpPacket packet)
        {
            ...
            catch(DuplicatePacketException)
            {
                ...
                eventLog.WriteEntry(string.Format(CultureInfo.CurrentCulture, 
                    Strings.DuplicatePacketReceivedInRtpStream, packet), EventLogEntryType.Error, 
            ...
            if (frame.Complete)
            {
                ...
                //  RtpSenders increment by 1, to do otherwise is a signal from the Sender
        ...
        internal void AddReceiverReport(CompoundPacketBuilder cpb)
        {
            ...
            rr.FractionLost = (byte)((float)packetsLost / (packetsReceived + packetsLost) * 100F);
...
//            rr.ExtendedHighestSequence = ?;
        ...
        private float PacketsLostIn10000
        {
            get
            {
                if (packetsLost == 0) {
                    ...
                    return (float)packetsLost / (packetsReceived + packetsLost) * 10000F;
                }
            }
        }
        ...
        private float FramesLostIn10000
        {
            get
            {
                if (framesLost == 0)
                    ...
                    return (float)framesLost / (framesReceived + framesLost) * 10000F;
            }
        }
        ...
        internal void RaisePacketOutOfSequenceEvent(int packetsLost, string message)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaisePacketOutOfSequenceEvent, args);
        }
        ...
        internal void RaiseInvalidPacketInFrameEvent(string reason)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseInvalidPacketInFrameEvent, args);
        }
        ...
        internal void RaiseFrameOutOfSequenceEvent(int framesLost, string message)
        {
            ...
            EventThrower.QueueUserWorkItem(RtpEvents.RaiseFrameOutOfSequenceEvent, args);
        }
        ...
        internal void OnFrameReceivedEvent(BufferChunk frame)
        {
            ...
            if(FrameReceived != null)
            {
                ...
                EventThrower.QueueUserWorkItem(RaiseFrameReceivedEvent, args);
            }
        ...
        internal void OnFirstFrameReceivedEvent()
        {
            ...
            EventThrower.QueueUserWorkItem(RaiseFirstFrameReceivedEvent, args);
        }
        ...
        {
            object[] args = {this, null};
            EventThrower.QueueUserWorkItem(RaiseDataStartedEvent, args);
        }
        ...
        {
            object[] args = {this, null};
            EventThrower.QueueUserWorkItem(RaiseDataStoppedEvent, args);
        }
    ...
    internal abstract class RtpStreamFec : RtpStream
    {
        ...
        private RtpListener.GetBufferHandler getBufferHandler;
        ...
        {
            getBufferHandler = rtpListener.GetBufferCallback;
        }
