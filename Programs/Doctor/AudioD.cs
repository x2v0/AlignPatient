// $Id: AudioD.cs 1889 2013-05-23 05:52:31Z onuchin $
...
{
   public partial class DoctorDisplayForm : Form
   {
      #region Members

      #region Sending side / Capture graph

      protected AudioCaptureGraph fAudioCaptureGraph = null;
      protected AVLogger fLog = null;

      /// <summary>
      /// Audio microphone filter info
      /// </summary>
      protected FilterInfo fMicroponeFI;
      ...
      protected FilgraphManagerClass fgmAudio = null;
      ...
      public void ActivateMicrophone()
      {
         ...
         Log(string.Format(CultureInfo.CurrentCulture, "\r\nInitializing microphone - {0}, {1}", 
             fMicroponeFI.DisplayName, fMicroponeFI.Moniker));
#endif
         // Get microphone up and running
         CreateAudioCaptureGraph(fMicroponeFI);
      ...
      private void RestoreAudioSettings()
      {
         ...
         byte[] bytes = (byte[])Reg.ReadValue(DeviceKey(), Reg.MediaType);
         ...
         if (bytes != null) {
            ...
            Reg.ms.Position = 0;
            _AMMediaType mt = (_AMMediaType)Reg.bf.Deserialize(Reg.ms);
         ...
         lock (fgmAudioLock) {
            if(fgmAudioState != FilterGraph.State.Running) {
               ...
               if (fgmAudio != null) {
                  ...
                  // Note: use the currentVolume member to set the volume.
                  ...
                  // manually block the stream to reset its state in case it
      ...
      private void CreateAudioRenderGraph()
      {
         lock (fgmAudioLock) {
            ...
            ((IRtpSource)rtpSource).Initialize(fRtpAudioStream);
            ...
            var rtpOut = Filter.GetPin(rtpSource, _PinDirection.PINDIR_OUTPUT, Guid.Empty, Guid.Empty, false, 0);

            var noise = Filter.CreateBaseFilterByName("Sony Wave Hammer");
            //iGB.AddFilter(noise, "Sony Wave Hammer");
            //var inputPin = Filter.GetPinByName(noise, "Input");
            //var outputPin = Filter.GetPinByName(noise, "Output");
            //iGB.Connect(rtpOut, inputPin);
            iGB.Render(rtpOut);
